...
class SimplifiedLassoController : public EntityController
{
...
public:
    ...
    SimplifiedLassoController(Entity robot, std::shared_ptr<World> world, default_random_engine &rng, Config &config)
        : m_world(world)
        , m_robot(robot)
        , m_rng(rng)
        , m_config(config)
